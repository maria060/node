...
select libri.titolo, group_concat(autori.cognome) as autori from libri
    left join autori_libri on libri.id_libro=autori_libri.id_libro
    left join autori on autori.id_autore=autori_libri.id_autore
where libri.id_libro = 3
group by libri.id_libro;
select '';


SELECT libri.id_libro, libri.titolo, group_concat(autori.cognome) as autori FROM libri
    LEFT JOIN autori_libri ON libri.id_libro=autori_libri.id_libro
    LEFT JOIN autori ON autori_libri.id_autore=autori.id_autore
GROUP BY libri.id_libro
